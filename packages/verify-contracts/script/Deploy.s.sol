...
/// @notice Deploy the Guard. Env: DEPLOYER_PRIVATE_KEY, ATTESTATION_SIGNER, GUARD_OWNER (defaults to deployer),
///         SIGNER_EPOCH (default 1), MAX_CERT_TTL_SECONDS (default 120).
///         Allowlists are configured separately by Configure.s.sol after operator approval (硬约束①).
contract Deploy is Script {
    function run() external returns (ChaconneVerifyGuard guard) {
        ...
        guard = new ChaconneVerifyGuard(owner, signer, epoch, ttl);
        vm.stopBroadcast();

        console2.log("ChaconneVerifyGuard deployed at", address(guard));
