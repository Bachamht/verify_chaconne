...
const ZH_EN: Record<string, string> = {
  ...
  "mandate.effectivePolicyHash 与任务授权范围（scopeHash）不一致：范围不可改，放宽范围请建新任务（CV-D16）": "mandate.effectivePolicyHash does not match the task's signed scope (scopeHash): the scope cannot change; create a new task to widen it (CV-D16)",
  "授权范围校验失败": "scope validation failed",
  "交易意图校验失败": "trade intent validation failed",
  "简报校验失败": "brief validation failed",
  "agent 状态回报校验失败": "agent status report validation failed",
  "该任务没有授权范围（建于 CV-D16 之前）：请建新任务再提交意图": "this task has no signed scope (created before CV-D16): create a new task to submit intents",
  "该任务没有授权范围（建于 CV-D16 之前）": "this task has no signed scope (created before CV-D16)",
  "任务已暂停：暂停期间不按意图签发（D-088）": "task is paused: no certificate is issued on intents while paused (D-088)",
  "授权范围必须包住计划": "the scope must contain the plan (assets, budget, per-step cap, steps, deadline)",
  "该任务由 agent 提交交易意图后签发（scope.issuance=agent）；prepare-step 不按计划签发": "this task issues only on agent-submitted trade intents (scope.issuance=agent); prepare-step does not issue by plan",
