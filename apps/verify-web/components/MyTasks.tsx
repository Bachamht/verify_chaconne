...
import { clearHistory, forget, history, hrefFor, isAgentTask, type HistoryItem } from "@/lib/history";
import { fmtLocal } from "@/lib/format";
import { X } from "lucide-react";
import { Card, EmptyState, Pill } from "@/components/ui";

const KIND_TONE: Record<HistoryItem["kind"], "ok" | "warn" | "brand" | "neutral"> = { job: "ok", plan: "brand", mandate: "warn", simulation: "neutral", agent_task: "brand" };
          ...
          <ul className="divide-y divide-line text-sm">
            {items.map((it) => (
              <li key={`${it.kind}:${it.id}`} className="flex flex-wrap items-center gap-3 py-2">
                <Pill tone={isAgentTask(it) ? "brand" : KIND_TONE[it.kind]}>{isAgentTask(it) ? (locale === "zh" ? "计划任务" : "Agent task") : t(`me_kind_${it.kind}` as "me_kind_job")}</Pill>
