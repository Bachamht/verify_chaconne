    ...
    <header className="verify-header" ref={headerRef}>
      ...
      {open && <nav id="verify-browse-nav" className="verify-browse-nav" aria-label={locale === "zh" ? "探索 Chaconne Agent" : "Explore Chaconne Agent"}>
        <div className="verify-browse-inner">
          ...
          <div className="verify-browse-settings">
            <button className="btn-ghost h-9 px-3 text-xs" aria-pressed={demo} onClick={() => setDemo(!demo)}>{t("demo_mode")}<span className={demo ? "text-brand-300" : "text-fg-3"}>{demo ? "ON" : "OFF"}</span></button>
            <div className="verify-browse-status"><ModeBadge /><ChainBadge /></div>
          </div>
        </div>
      </nav>}
    </header>
