...
const E: Record<string, { en: string; zh: string }> = {
  ...
  wallet_signin_required: { en: "Sign in with your wallet first (one signature, no transaction).", zh: "请先用钱包登录（签一条消息，不是交易）。" },
  wallet_signin_rejected: { en: "Sign-in was cancelled or no wallet is connected; this wallet's records stay hidden until you sign in.", zh: "登录签名被取消或没有连接钱包；登录前看不到这个钱包的记录。" },
  missing_api_key: { en: "This call needs an API key. Issue one under Agent API keys.", zh: "这个调用需要 API key，在「Agent 接入 key」生成。" },
  invalid_api_key: { en: "Unknown or revoked API key.", zh: "API key 不存在或已吊销。" },
  bad_signature: { en: "The signature does not match this wallet.", zh: "签名与这个钱包不匹配。" },
  nonce_reused: { en: "That signed message was already used; sign again.", zh: "这条签名已经用过了，请重新签一条。" },
  issued_at_out_of_window: { en: "Your device clock differs from the server by more than 10 minutes.", zh: "你设备的时钟与服务器相差超过 10 分钟。" },
  too_many_keys: { en: "This wallet already holds the maximum number of keys; revoke one first.", zh: "这个钱包的 key 已达上限，先吊销一把。" },
  key_not_found: { en: "Key not found, or it belongs to another wallet.", zh: "找不到这把 key，或它属于另一个钱包。" },
  session_not_configured: { en: "Wallet sign-in is not configured on this deployment.", zh: "这个部署还没配置钱包登录。" },
  nonce_expired: { en: "The sign-in challenge expired; try again.", zh: "登录挑战已过期，请再试一次。" },
